...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [toEmail],
      subject: 'New Appointment Request - VetCare Family',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            ...
            <p style="margin: 10px 0 0 0; font-size: 16px; opacity: 0.9;">New Appointment Request</p>
          </div>
          
          <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e2e8f0;">
            <h2 style="color: #1e293b; margin-top: 0; font-size: 22px;">Client Details:</h2>
            
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
              <p style="margin: 0 0 10px 0;"><strong style="color: #1e40af;">Name:</strong> ${name}</p>
              <p style="margin: 0 0 10px 0;"><strong style="color: #1e40af;">Phone:</strong> <a href="tel:${phone}" style="color: #3b82f6; text-decoration: none;">${phone}</a></p>
            ...
            ${message ? `
              <h3 style="color: #1e293b; margin: 25px 0 15px 0; font-size: 18px;">Message:</h3>
            ...
            <div style="margin-top: 30px; padding: 20px; background: #eff6ff; border-radius: 8px; border: 1px solid #bfdbfe;">
              <h3 style="color: #1e40af; margin: 0 0 10px 0; font-size: 16px;">Recommended Actions:</h3>
              <ul style="margin: 0; padding-left: 20px; color: #374151;">
                <li>Contact the client as soon as possible</li>
                <li>Check availability for appointment</li>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f1f5f9; border-radius: 8px; text-align: center;">
              <p style="margin: 0; font-size: 14px; color: #64748b;">
                This request was sent from the VetCare Family website<br>
                Date: ${new Date().toLocaleString('en-US', { timeZone: 'Europe/Bucharest' })}
              </p>
            </div>
          </div>
        </div>
